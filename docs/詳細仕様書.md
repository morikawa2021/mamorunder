...
class Task: NSManagedObject {
    ...
    @NSManaged var snoozeMaxCount: Int        // スヌーズ最大回数（デフォルト5、最大10）
    @NSManaged var snoozeUnlimited: Bool      // 無期限スヌーズ（期限超過時）
    @NSManaged var snoozeCount: Int           // 現在のスヌーズ回数
    @NSManaged var lastSnoozeDateTime: Date?  // 最後のスヌーズ日時（リセット判定用）
